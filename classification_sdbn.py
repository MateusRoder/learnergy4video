...
def exec_class(name, seed):
    ...
    try:
        ...
        #in_shape = model.hidden_shape[0] * model.hidden_shape[1] * model.n_filters[tam-1]
    except:
    ...
    try:
        for i in range(1, tam):
            ...
            #pass
            optimizer.append(optim.Adam(model.parameters(), lr=1e-5)) #Estudar 10e-7
    ...
    train_batch = DataLoader(train, batch_size=batch_size, drop_last=True, shuffle=True, num_workers=workers, collate_fn=collate_fn)
    ...
    for e in range(fine_tune_epochs):
        ...
        for x_batch, y_batch in train_batch:
            ...
            y = model(x_).detach()
            yt = model(x_t).detach()
            #print("Y", y.size(), in_shape)
            # Calculating the fully-connected outputs
            y = y.reshape(y.size(0), in_shape)
    ...
    val_batch = DataLoader(test, batch_size=batch_size, drop_last=True, shuffle=True, num_workers=workers, collate_fn=collate_fn)
    val_acc = []
...
#exec_class('U1spec_cdbn0.pth', 0)
#exec_class('tuned_1spec_cdbn0.pth', 0)
