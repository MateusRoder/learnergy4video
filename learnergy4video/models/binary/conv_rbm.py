...
class ConvRBM(Model):
    ...
    @b.setter
    def b(self, b):
        #if not isinstance(b, (torch.tensor, float)):
    ...
    def optimizer(self, optimizer):
        if not isinstance(optimizer, (opt.SGD, opt.Adam)):
            raise e.TypeError('`optimizer` should be a valid Pytorch optimizer')
    ...
    def hidden_sampling(self, v):
        ...
        activations = F.conv2d(v, self.c1*self.W, bias=self.b)
    ...
    def visible_sampling(self, h):
        ...
        # Calculating neurons' activations
        activations = F.conv_transpose2d(h, self.c2*self.W, bias=self.a)
