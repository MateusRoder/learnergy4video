...
class CDBN(Model):
    ...
    def __init__(self, visible_shape=(28, 28), filter_shape=[(7, 7), (7, 7)], n_filters=[16, 16], n_channels=1,
        ...
        for i in range(self.n_layers):
            ...
            #visible_shape = ( (m.hidden_shape[0]//2) + 1, (m.hidden_shape[1]//2) + 1) # FOR MAX-POOLING 
    ...
    def fit(self, dataset, batch_size=128, epochs=(10, 10)):
        ...
        for i, model in enumerate(self.models):
            ...
            if i == 0:
                try:
                    model = torch.load("std_cdbn_model.pth")
                    model.eval()
                    model.cuda()
                    self.models[0] = model
                    continue
            ...
            for epoch in range(epochs[i]):
                ...
                for samples, _ in tqdm(batches):
                    ...
                    for fr in range(frames):
                        ...
                        if i == 0:
                            # Fits the first CRBM
                            mse_ += model.fit(sp, len(sp), 1)
                        else:
                            sp = sp.reshape(
                                len(sp), self.n_channels, self.visible_shape[0], self.visible_shape[1])

                            sp = ((sp - torch.mean(sp, 0, True)) / (torch.std(sp, 0, True) + 1e-6))
                            ...
                            for md in range(i):
                                sp, _ = self.models[md].hidden_sampling(sp)
                                #sp = self.max_pooling(sp)
            ...
            if i == 0:
                torch.save(model, "std_cdbn_model.pth")
