...
from learnergy4video.models.real import (GaussianRBM, SigmoidRBM, SpecRBM)
...
class SpecDBM(Model):
    """A SpecDBM class provides the basic implementation for Spectral Deep Boltzmann Machines.
    References:
        .
    """
    ...
    def __init__(self, model='gaussian', n_visible=128, n_hidden=(128,), steps=(1,),
        ...
        logger.info('Overriding class: Model -> SpecDBM.')

        # Override its parent class
        super(SpecDBM, self).__init__(use_gpu=use_gpu)
            ...
            if i == 0:
                ...
                for ep in range(epochs[i]): # iterate over epochs for model 'i'
                    ...
                    for x, y in tqdm(batches):
                        ...
                        model_mse, model_pl, ct = model.fit(x, len(x), 1, 1)
    ...
    def fit(self, dataset, batch_size=128, epochs=10, frames=6):
        ...
        for e in range(epochs):
            ...
            for samples, y in tqdm(batches):
                ...
                visible_states = self.gibbs_sampling(samples)
                ...
                batch_mse = torch.div(
                        torch.sum(torch.pow(samples - visible_states[0], 2)), batch_size2).detach()
